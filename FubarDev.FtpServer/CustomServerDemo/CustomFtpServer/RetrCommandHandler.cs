namespace CustomServerDemo.CustomFtpServer
{
    ...
    public class RetrCommandHandler : FtpCommandHandler
    {
        ...
        public RetrCommandHandler(ILogger<RetrCommandHandler>? logger = null)
        {
            _logger = logger;
        }
        ...
        public override async Task<IFtpResponse?> Process(FtpCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                await FtpContext.ServerCommandWriter
                    .WriteAsync(new SendResponseServerCommand(new FtpResponse(150, T("Opening connection for data transfer."))), cancellationToken)
                    .ConfigureAwait(false);
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                int receivedBytes;
                while ((receivedBytes = await input.ReadAsync(buffer, 0, buffer.Length, cancellationToken).ConfigureAwait(false)) != 0)
                {
                    Console.WriteLine("{0:P}", input.Position / (double)input.Length);
                    await stream.WriteAsync(buffer, 0, receivedBytes, cancellationToken).ConfigureAwait(false);
                }
            }
